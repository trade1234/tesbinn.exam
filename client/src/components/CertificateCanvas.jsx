...
export function drawCertificate(canvas, c, logo, qr) {
  ...
  ctx.save();ctx.shadowColor="rgba(0,63,160,.38)";ctx.shadowBlur=14;ctx.shadowOffsetY=7;ctx.beginPath();
  for(let i=0;i<=badgeSamples;i++){const angle=-Math.PI/2+i*Math.PI*2/badgeSamples,radius=badgeRadius+6*Math.cos(angle*12),x=badgeX+Math.cos(angle)*radius,y=badgeY+Math.sin(angle)*radius;if(i===0)ctx.moveTo(x,y);else ctx.lineTo(x,y)}
  ctx.closePath();const badgeGradient=ctx.createRadialGradient(badgeX-16,badgeY-18,5,badgeX,badgeY,badgeRadius+8);badgeGradient.addColorStop(0,"#55a0ff");badgeGradient.addColorStop(.48,"#247be8");badgeGradient.addColorStop(1,"#0754bd");ctx.fillStyle=badgeGradient;ctx.fill();ctx.restore();
  ctx.lineCap="round";ctx.lineJoin="round";ctx.strokeStyle="rgba(0,46,126,.38)";ctx.lineWidth=11;ctx.beginPath();ctx.moveTo(1348,176);ctx.lineTo(1363,191);ctx.lineTo(1395,156);ctx.stroke();ctx.strokeStyle="#f2f4f7";ctx.lineWidth=10;ctx.beginPath();ctx.moveTo(1346,173);ctx.lineTo(1362,189);ctx.lineTo(1393,154);ctx.stroke();ctx.textAlign="center";ctx.fillStyle="#0754bd";ctx.font="700 14px Arial";ctx.fillText("VERIFIED",badgeX,244);
  ...
  ctx.textAlign="center";ctx.strokeStyle="#64748b";ctx.lineWidth=1;ctx.beginPath();ctx.moveTo(630,875);ctx.lineTo(970,875);ctx.stroke();ctx.fillStyle="#13294b";ctx.font="italic 25px Georgia";ctx.fillText(c.signatoryName||"Authorized Certification Officer",800,865,320);ctx.font="16px Arial";ctx.fillStyle="#64748b";ctx.fillText("Digitally Signed & Verified",800,900);
  ...
  if(qr){ctx.fillStyle="#ffffff";ctx.strokeStyle="#d7e5f5";ctx.lineWidth=2;ctx.beginPath();ctx.roundRect(748,915,104,104,10);ctx.fill();ctx.stroke();ctx.drawImage(qr,755,922,90,90);ctx.textAlign="center";ctx.fillStyle="#0754ad";ctx.font="700 13px Arial";ctx.fillText("SCAN TO VERIFY",800,1042);}
  center("Verified professional credential - "+(c.enrollmentNumber||"Trade Ethiopia Certification"),1080,"16px Arial","#64748b",800);
}
