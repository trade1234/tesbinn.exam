  ...
  useEffect(() => {
    api.get("/certificates").then(({ data }) => {
      ...
      if (certificate?._id) {
        ...
        navigate(`/student/certificates/${certificate._id}`, { replace: true, state: { certificate } });
      }
    }).catch(() => {});
  }, [attemptId, navigate]);
  ...
  return <div className="mx-auto max-w-5xl space-y-6">
    <div className="rounded-2xl bg-white p-5 shadow-soft dark:bg-[#111a2b] sm:flex sm:items-center sm:justify-between sm:p-7">
      ...
      <div className="mt-5 flex flex-col gap-3 sm:mt-0 sm:flex-row">
        ...
        <button className="btn-secondary" type="button" onClick={downloadImage} disabled={downloading}><Download size={17} /> {downloading ? "Preparing image..." : "Download result image"}</button>
      </div>
    </div>
